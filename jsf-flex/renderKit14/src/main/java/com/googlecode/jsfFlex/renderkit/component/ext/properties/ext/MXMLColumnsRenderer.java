...
public final class MXMLColumnsRenderer extends MXMLComponentBaseRenderer {
	...
	public void encodeEnd(FacesContext context, UIComponent componentObj) throws IOException {
		...
		if(childrenList.size() > 0){
			...
			if(maxColumnDataSize < batchColumnDataRetrievalSize.intValue()){
				batchColumnDataRetrievalSize = Integer.valueOf(maxColumnDataSize);
			}
			
			Integer maxDataPartitionIndex = Integer.valueOf((int) Math.ceil( maxColumnDataSize / batchColumnDataRetrievalSize.intValue() ));
